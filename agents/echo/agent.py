...
class EchoAgent(BaseAgent):
    ...
    def process_message(self, message: str, **kwargs) -> str:
        """Process reading comprehension request"""
        try:
            reading_level = kwargs.get("reading_level", "middle")
            text_to_analyze = kwargs.get("text", None)
            
            # Filter message for safety
            is_safe, safety_msg = self.safety_filter.validate_student_input(message)
            if not is_safe:
                return safety_msg
            ...
            # Determine intent
            intent = self._determine_intent(message, text_to_analyze)
            
            # Process based on intent
            if intent == "analyze_text":
                response = self._analyze_text(text_to_analyze or message, reading_level)
            elif intent == "vocabulary_breakdown":
                response = self._provide_vocabulary_breakdown(self.current_text or message, reading_level)
            elif intent == "create_questions":
                response = self._create_comprehension_questions(self.current_text or message, reading_level)
            elif intent == "rewrite_help":
                response = self._help_rewrite(self.current_text or message, reading_level)
            elif intent == "summary_request":
                response = self._create_summary(self.current_text or message, reading_level)
            else:
                response = self._general_comprehension_help(message, reading_level)
